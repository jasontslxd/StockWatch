...
export const TickerLogo: React.FC<ITickerLogoProps> = ({ ticker, renderLoading = false, className = ''}) => {
  ...
  if (isLoadingTickerProfile || renderLoading) {
    return (
      <div style={{
        ...
        <Placeholder animation="wave" className="rounded-circle w-100 h-100" xs={3}>
          <Placeholder xs={12} />
        </Placeholder>
      </div>
    );
