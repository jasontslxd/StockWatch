...
export const OtpSubmission: React.FC<IOtpSubmissionProps> = ({ flow, setOtpValidationError }) => {
  ...
  return (
    <Container style={{top: '60vh'}} className="end-0 start-0 position-absolute">
      ...
      <div className="d-flex justify-content-center">
        <p className="text-secondary text-decoration-underline" onClick={() => {window.alert('Resend OTP')}}>Resend OTP</p>
      </div>
      <Spacer size="lg" />
      <div className="d-flex justify-content-center">
        <Button variant="danger rounded-pill w-75" onClick={onSubmitOtp} disabled={isLoading}>{isLoading ? <Spinner animation="border" variant="light" /> : submitButtonText}</Button>
      </div>
    </Container>
  );
};