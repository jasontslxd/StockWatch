...
export const TickerCard: React.FC<ITickerCardProps> = ({
  tickerPerformance,
}) => {
  ...
  return (
    <Card
      className="bg-white"
      onClick={() => navigate(Page.Instrument.replace(':ticker', ticker))}
    >
      <Card.Body className="p-2">
        <Card.Title>
          <TickerLogo ticker={ticker} />
        </Card.Title>
        <Card.Text className="fs-5 fw-bold m-0">{ticker}</Card.Text>
        <Card.Text className="fw-bold m-0">{parsedPrice}</Card.Text>
        <Spacer size="xxs" />
        <Card.Text>
          <ChangePercentage
